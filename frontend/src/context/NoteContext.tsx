  ...
  const fetchNotes = React.useCallback(async (): Promise<void> => {
    try {
      ...
      const response: AxiosResponse<NoteInterface[]> = await axios.get(
        `${process.env.NEXT_PUBLIC_BACKEND_URL}/get-all-notes`,
        {
          headers: {
  ...
  const addNote = async (title: string, content: string) => {
    try {
      const response = await axios.post(
        `${process.env.NEXT_PUBLIC_BACKEND_URL}/create-note`,
        { title, content },
  ...
  const editNote = async (editedNote: NoteInterface) => {
    try {
      ...
      const response = await axios.put(
        `${process.env.NEXT_PUBLIC_BACKEND_URL}/update-note/${editedNote._id}`,
        { title, content },
        { headers: { Authorization: token ? `Bearer ${token}` : '' } },
      )
  ...
  const deleteNote = async (noteId: string) => {
    try {
      const response = await axios.delete(
        `${process.env.NEXT_PUBLIC_BACKEND_URL}/delete-note/${noteId}`,
        { headers: { Authorization: token ? `Bearer ${token}` : '' } },
      )
